...
'''
'''agenda_testes1 = {'joao':22222222, 'jose':33333333}
'''
...
def sem_email(agenda):
    ...
    for nome in agenda:
        if 'email' not in agenda[nome]:
            semEmail.append(nome)
...
def conta_telefones(agenda):
    ...
    for nome, info in agenda.items():
        if 'telefones' in info:
            total_telefones += len(info['telefones'])
    
    return total_telefones
