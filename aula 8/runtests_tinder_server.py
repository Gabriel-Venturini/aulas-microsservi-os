...
p4: preferencias
    ...
    as caracteristicas sexo e buscando afetam quem pode sinalizar interesse. 
    Sinalizações inválidas resultam cod status 400
    Também retornam um dicionário, mas vou complicar a sua vida.
    Tenta entender a partir do teste que dicionário é esse :P
'''
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_p1_00_pessoas_retorna_lista(self):
        ...
        r = requests.get(f'http://{localhost}:5003/pessoas')
    ...
    def test_p1_01_adiciona_pessoas(self):
        r_reset = requests.post(f'http://{localhost}:5003/reseta')
        ...
        r = requests.post(f'http://{localhost}:5003/pessoas',
        ...
        #crio roberto
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'roberto','id':2})
        if r.status_code != 200:
            try:
                print('erro',r.json())
            except:
        ...
        #roberto e fernando
        r_lista = requests.get(f'http://{localhost}:5003/pessoas')
    ...
    def test_p1_02_pessoa_por_id(self):
        ...
        r = requests.get(f'http://{localhost}:5003/pessoas/20')
    ...
    def test_p1_03_reseta_apaga_pessoas(self):
        #crio um usuário
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'cicero','id':29})
        ...
        r_lista_depois = requests.get(f'http://{localhost}:5003/pessoas')
    ...
    def test_p2_00_interesse_com_pessoas_validas(self):
        ...
        #crio a pessoa 9
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'maximus','id':9})
        self.assertEqual(r.status_code,200)

        #interesse de 9 para 3 dá erro, pois 3 nao existe ainda
        r = requests.put(f'http://{localhost}:5003/sinalizar_interesse/9/3/')
        self.assertEqual(r.status_code,404)

        #interesse de 3 para 9 dá erro, pois 3 nao existe ainda
        r = requests.put(f'http://{localhost}:5003/sinalizar_interesse/3/9/')
        self.assertEqual(r.status_code,404)
        
        #agora crio a pessoa 3
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'aurelia','id':3})
        self.assertEqual(r.status_code,200)
        ...
        r = requests.put(f'http://{localhost}:5003/sinalizar_interesse/9/9/')
        self.assertEqual(r.status_code,200)
    ...
    def test_p2_01_consulta_interesse(self):
        ...
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'maximus','id':9})
        self.assertEqual(r.status_code,200)
        r = requests.get(f'http://{localhost}:5003/interesses/9')
        self.assertEqual(r.status_code,200)
        ...
        #crio diana
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'diana','id':30})
        self.assertEqual(r.status_code,200)
        
        #maximus está interessado em diana
        #(veja na URL, estamos marcando interesse de 9 para 30)
        r = requests.put(f'http://{localhost}:5003/sinalizar_interesse/9/30/')
        self.assertEqual(r.status_code,200)
        r = requests.get(f'http://{localhost}:5003/interesses/9')
        self.assertEqual(r.status_code,200)
        lista_interesses = r.json() 
    ...
    def test_p2_04_deleta_interesse(self):
        ...
        requests.delete(f'http://{localhost}:5003/sinalizar_interesse/9/30/')
        ...
        #deletei o interesse de 9 em 30
        requests.delete(f'http://{localhost}:5003/sinalizar_interesse/9/3/')
        #consultei os interesses de 9
        r = requests.get(f'http://{localhost}:5003/interesses/9')
        lista_interesses = r.json() 
    ...
    def test_p3_01_match(self):
        ...
        r = requests.post(f'http://{localhost}:5003/pessoas',json={'nome':'maximus','id':9})
        self.assertEqual(r.status_code,200)
        r = requests.get(f'http://{localhost}:5003/matches/9')
        self.assertEqual(r.status_code,200)
        ...
        r = requests.put(f'http://{localhost}:5003/sinalizar_interesse/30/9/')
        self.assertEqual(r.status_code,200)
        r = requests.get(f'http://{localhost}:5003/matches/9')
        self.assertEqual(r.status_code,200)
        ...
        r = requests.get(f'http://{localhost}:5003/matches/3')
        self.assertEqual(r.status_code,200)
        self.assertEqual(r.json(),[9])

        #diana também tem o match
        r = requests.get(f'http://{localhost}:5003/matches/30')
        self.assertEqual(r.status_code,200)
        self.assertEqual(r.json(),[9])
    ...
    def test_p3_02_match_404(self):
        ...
        r = requests.get(f'http://{localhost}:5003/matches/4')
        self.assertEqual(r.status_code,404)
    ...
    def test_p3_03_match_perdido(self):
        ...
        r = requests.delete(f'http://{localhost}:5003/sinalizar_interesse/3/9/')
        self.assertEqual(r.status_code,200)
    ...
    def test_p4_01_match_incompativel(self):
        ...
        self.assertEqual(r.status_code,404)
        maximus = {'nome':'maximus','id':9,'sexo':'homem','buscando':['mulher']}
        r = requests.post(f'http://{localhost}:5003/pessoas',json=maximus)
        self.assertEqual(r.status_code,200)
        ...
        r = requests.post(f'http://{localhost}:5003/pessoas',json=aurelia)
        self.assertEqual(r.status_code,200)
